"""
PepeRush Bot — Database Layer (SQLite)
All DB access is synchronous and wrapped in thread-safe helpers.
"""
    ...
    with get_conn() as conn:
        conn.executescript("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                wallet          TEXT,
                joined_channels INTEGER DEFAULT 0,   -- 1 = verified join wall
                human_verified  INTEGER DEFAULT 0,   -- 1 = passed captcha
            ...
            CREATE TABLE IF NOT EXISTS tasks (
                ...
                chat_id         TEXT,               -- Telegram chat_id for getChatMember
...
def _seed_default_tasks(conn):
    """Insert the hardcoded required channels/groups on first run."""
    defaults = [
        # (platform, link, chat_id)
        ...
        ("telegram", "https://t.me/+8SimTcYDCJllMjI0",           None),
        ("telegram", "https://t.me/+H6oH96FgLi44ZDVk",           None),  # payout ch
        ("whatsapp", "https://chat.whatsapp.com/INAiFm6tpDMLmdq0OkLoKz?mode=gi_t", None),
...
def set_joined_channels(user_id: int):
    with get_conn() as conn:
        conn.execute("UPDATE users SET joined_channels=1 WHERE user_id=?", (user_id,))
...
def get_stats() -> dict:
    with get_conn() as conn:
        ...
        total_pepe_out = conn.execute(
            "SELECT COALESCE(SUM(amount),0) as s FROM withdrawals WHERE status='done'"
        ).fetchone()["s"]
        return {
            "total_users": total_users,
            "total_withdrawals": total_withdrawals,
            "pending_withdrawals": pending_withdrawals,
            "total_pepe_out": total_pepe_out,
        }
...
def mark_referral_rewarded(new_user_id: int, referrer_id: int):
    with get_conn() as conn:
        conn.execute(
            "INSERT OR IGNORE INTO referral_rewarded (new_user_id) VALUES (?)", (new_user_id,)
        )
        conn.execute(
            "DELETE FROM referral_pending WHERE new_user_id=?", (new_user_id,)
        )
        conn.execute(
...
def update_task_chat_id(task_id: int, chat_id: str):
    with get_conn() as conn:
        conn.execute("UPDATE tasks SET chat_id=? WHERE id=?", (chat_id, task_id))
